...
mod tests {
    ...
    #[test]
    fn test_dp() {
        assert_eq!(egg_drop2(par_simple_dp, 8, 8), 4); // useful for debugging
        assert_eq!(egg_drop2(par_simple_dp, 2, 1), 1);
        assert_eq!(egg_drop2(par_simple_dp, 1, 2), 2);
        assert_eq!(egg_drop2(par_simple_dp, 2, 6), 3);
        assert_eq!(egg_drop2(par_simple_dp, 3, 14), 4);
        assert_eq!(egg_drop2(par_simple_dp, 4, 30), 5);
        assert_eq!(egg_drop2(par_simple_dp, 5, 62), 6);
        assert_eq!(egg_drop2(par_simple_dp, 6, 126), 7);
        assert_eq!(egg_drop2(par_simple_dp, 50, 500), 9);
        assert_eq!(egg_drop2(par_simple_dp, 10, 8), 4);
        assert_eq!(egg_drop2(par_simple_dp, 50, 500), 9);
    }
}
